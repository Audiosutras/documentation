...
2. Run the project detached as a background process.
   ...
   If you are running this project on a raspberry pi or a device using the arm64 architecture you will need to modify the `unbound.image` value in the `docker-compose.yml` file.
   Change `mvance/unbound:latest` to `alpinelinux/unbound:latest-aarch64`. Save the file and then run `docker-compose up -d`.
